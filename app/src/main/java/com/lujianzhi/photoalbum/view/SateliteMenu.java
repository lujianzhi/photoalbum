...
/**
 * 卫星式菜单 author : stone email : aa86799@163.com time : 15/5/7 14 17
 */
...
public class SateliteMenu extends ViewGroup implements View.OnClickListener {
    ...
    @Override
    // lt 左上点 rb 右下点 如果 r<l 或 b<t 则无法显示了
    protected void onLayout(boolean changed, int l, int t, int r, int b) {// l=0,
        // t=0
        // 因为是相对于父view的位置
        layoutMenuButton();
		/*
		 * 分析： menuButton距离每个item为radius。
		 * 到item作直线，其夹角，应为90度均分。90/(item-1)=每个夹角的度数。 有角度，就能求出正弦值sina。
		 * 根据正弦公式：sina=a/c，且已知c=radius，求出a边长，即x坐标。 有角度，就能求出正弦值cosa。
        ...
        for (int i = 1; i < count; i++) {
            ...
            double sin = 0, cos = 0;
            // Math.toRadians：math.pi/180 * angle = 弧度
            // angel/180*pi<==>angel*pi/180
            sin = Math.sin(Math.toRadians(angle * (i - 1))); // 第i个角度的 sin(0)=0
            // i-1即从0开始,会有与屏幕直角边平行的
            // math.sin需要传弧度值
    ...
    private void toggleMenu(int duration) {
        ...
        for (int i = 1; i < count; i++) {
			...
			/*
			 * 平移动画 以layout中计算的长度 再乘以1或-1 close： 左上 r->l b->t 右上 l->r b->t 左下
			 * r->l t->b 右下 l->r t->b open： 左上 右上 左下 右下
			 */
            int xflag = 1, yflag = 1;
            //
			 ...
			 */
            int oppositeLen = (int) (mRadius * Math.sin(Math.PI / 2 / (count - 2) * (i - 1))); // 对边
            // 横向
            int adjacentLen = (int) (mRadius * Math.cos(Math.PI / 2 / (count - 2) * (i - 1))); // 邻边
            // 纵向
			/*
			 * 一个圆的弧度是2π,角度是360° π/180，每角度对应的弧度 然后乘以角度数=其所对应的弧度
			 */
            // int oppositeLen = (int) (mRadius * Math.sin(angle * Math.PI / 180
            // * (i-1))); //对边 横向
            // int adjacentLen = (int) (mRadius * Math.cos(angle * Math.PI / 180
            // * (i-1))); //邻边 纵向
